...
export const ObservationsWorldMap = forwardRef<ObservationsWorldMapHandle, ObservationsWorldMapProps>(
  ...
  useEffect(() => {
    ...
    L.tileLayer(tileUrl, {
      ...
      maxZoom: 19,
      maxNativeZoom: 16,
      // Esri's tile service sends CORS headers permitting this; without it,
    ...
    // top of the gray base — without this the map has no country/city
    // names on it. No pane override needed: Leaflet stacks tile layers
    // within the shared tilePane in insertion order.
    ...
    // Keep popup links inside the SPA instead of triggering a full reload. One
    // delegated listener on the map container — Leaflet reuses a bound popup's
    // DOM across open/close, so attaching per-anchor on every `popupopen` piled
    // up duplicate listeners and fired navigate() once per prior open.
    const onContainerClick = (ev: MouseEvent) => {
      const link = (ev.target as HTMLElement | null)?.closest<HTMLAnchorElement>('.nebulis-obs-link');
      if (!link) return;
      ev.preventDefault();
