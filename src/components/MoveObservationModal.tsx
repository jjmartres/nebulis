...
export function MoveObservationModal({ isOpen, onClose, objectId, date, displayName, formattedDate }: Props) {
  ...
  const moveError = moveMutation.error
    ? (moveMutation.error instanceof Error ? moveMutation.error.message : t('observationDetail.moveObservationModal.moveFailed'))
    : '';
  ...
  // Guards both Escape and backdrop-click while a move is in flight, matching
  // the Cancel button's own `disabled={isMoving}`.
  const handleModalClose = () => {
    if (!isMoving) onClose();
  };

  return (
    <Modal isOpen={isOpen} onClose={handleModalClose} title={t('observationDetail.moveObservationModal.title')} backdropClassName="bg-black/70">
      <div className={`w-full max-w-md mx-4 rounded-2xl border p-6 space-y-4 ${isDark ? 'bg-slate-900 border-slate-700' : 'bg-white border-slate-200 shadow-xl'}`}>
        <div className="flex items-center gap-3">
          ...
          <h3 className={`font-display font-semibold text-lg ${isDark ? 'text-white' : 'text-slate-900'}`}>
            {t('observationDetail.moveObservationModal.title')}
          </h3>
        </div>
        <p className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
          <Trans
            i18nKey="observationDetail.moveObservationModal.description"
            ns="observations"
            values={{ name: displayName, date: formattedDate }}
            components={{ 1: <span className={`font-semibold ${isDark ? 'text-white' : 'text-slate-900'}`} /> }}
          />
        </p>

        <div className="space-y-1.5">
          <label className={`text-xs font-medium ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>
            {t('observationDetail.moveObservationModal.searchLabel')}
          </label>
          <div className="relative">
            ...
            <input
              ...
              placeholder={t('observationDetail.moveObservationModal.searchPlaceholder')}
              autoFocus
                ...
                <div className="min-w-0">
                  ...
                  {matchedAlias && (
                    <div className={`truncate text-xs italic ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>
                      {t('observationDetail.moveObservationModal.akaAlias', { alias: matchedAlias })}
                    </div>
                  )}
                </div>
        ...
        {selectedTarget && (
          <div className={`rounded-lg p-3 text-sm ${isDark ? 'bg-accent-500/10 border border-accent-500/20' : 'bg-accent-50 border border-accent-100'}`}>
            <div className={`font-medium ${isDark ? 'text-accent-400' : 'text-accent-700'}`}>
              {t('observationDetail.moveObservationModal.movingTo', {
                target: `${selectedTarget.id}${selectedTarget.name && selectedTarget.name !== selectedTarget.id ? ` - ${selectedTarget.name}` : ''}`,
              })}
            </div>
